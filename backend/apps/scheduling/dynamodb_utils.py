...
def sync_user_to_employee(user_id):
    ...
    try:
        ...
        full_name = user_item.get(
            "full_name", "Unknown"
        )  # Default to 'Unknown' if not found
        events = get_employee_events(user_id)
        updated_events = []
        # Update the titles in each event to the new full name if exist
        for event in events:
...
def get_user_instance_from_dynamodb(user_id):
    ...
    try:
        ...
        print(
            f"Failed to fetch user {user_id} from DynamoDB: {e.response['Error']['Message']}"
        )
        return None
...
def delete_employee_from_dynamodb(user_id):
    ...
    try:
        ...
        print(
            f"Failed to delete user {user_id} from Employees table: {e.response['Error']['Message']}"
        )
    ...
    if action == "sync":
        ...
        if user_instance:
            ...
            sync_user_to_employee(
                user_id, full_name
            )  # Modify function signature accordingly
    elif action == "delete":
